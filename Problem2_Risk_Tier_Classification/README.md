...
The deployed `risk_tier_service.py` API requires a valid `X-API-Key` header on every endpoint except `/health` (see `src/fastapi_service/.env.example`), and `/risk-tier` returns a real, live-computed `top_reasons` field explaining that specific prediction -- see `CHANGELOG.md`'s 2026-08-25 "Authentication + explainability hardening" entry.
