...
- 📊 [Financial Impact Dashboard](https://rnanda19.github.io/AMEX_RiskIQ_Enterprise_Credit_Risk_Platform/01_Problem1_Credit_Scoring_PD_Prediction/reports/executive_reports/Financial_Impact_Dashboard.html) — interactive ROI, revenue-impact, and loss-reduction dashboard
- 📈 [Power BI Dashboard Preview](https://rnanda19.github.io/AMEX_RiskIQ_Enterprise_Credit_Risk_Platform/01_Problem1_Credit_Scoring_PD_Prediction/reports/powerbi_dashboard/PowerBI_Dashboard_Preview.html) — star-schema data model and dashboard preview
...
The deployed `main.py` API requires a valid `X-API-Key` header on every endpoint except `/health` (see `src/fastapi_service/.env.example`), and `/predict` returns a real, live-computed `top_reasons` field explaining that specific prediction -- see `CHANGELOG.md`'s 2026-08-25 "Authentication + explainability hardening" entry.
