...
[![CI](https://img.shields.io/github/actions/workflow/status/rnanda19/AMEX_RiskIQ_Enterprise_Credit_Risk_Platform/ci.yml?branch=main&label=CI&labelColor=2563EB)](https://github.com/rnanda19/AMEX_RiskIQ_Enterprise_Credit_Risk_Platform/actions/workflows/ci.yml) [![Lint & Format](https://img.shields.io/github/actions/workflow/status/rnanda19/AMEX_RiskIQ_Enterprise_Credit_Risk_Platform/code-quality.yml?branch=main&label=Lint%20%26%20Format&labelColor=0D9488)](https://github.com/rnanda19/AMEX_RiskIQ_Enterprise_Credit_Risk_Platform/actions/workflows/code-quality.yml) [![CodeQL Security Scan](https://img.shields.io/github/actions/workflow/status/rnanda19/AMEX_RiskIQ_Enterprise_Credit_Risk_Platform/codeql.yml?branch=main&label=CodeQL%20Security%20Scan&labelColor=7C3AED)](https://github.com/rnanda19/AMEX_RiskIQ_Enterprise_Credit_Risk_Platform/actions/workflows/codeql.yml) [![Docker Build & Run Verification](https://img.shields.io/github/actions/workflow/status/rnanda19/AMEX_RiskIQ_Enterprise_Credit_Risk_Platform/docker-verify.yml?branch=main&label=Docker%20Build%20%26%20Run%20Verification&labelColor=B45309)](https://github.com/rnanda19/AMEX_RiskIQ_Enterprise_Credit_Risk_Platform/actions/workflows/docker-verify.yml) [![Secrets Management Verification](https://img.shields.io/github/actions/workflow/status/rnanda19/AMEX_RiskIQ_Enterprise_Credit_Risk_Platform/vault-verify.yml?branch=main&label=Secrets%20Management%20Verification&labelColor=4F46E5)](https://github.com/rnanda19/AMEX_RiskIQ_Enterprise_Credit_Risk_Platform/actions/workflows/vault-verify.yml) [![Python 3.11](https://img.shields.io/badge/python-3.11-blue.svg?labelColor=0891B2)]() [![CRISP-DM](https://img.shields.io/badge/methodology-CRISP--DM-informational.svg?labelColor=CA8A04)]() [![License: All Rights Reserved](https://img.shields.io/badge/license-All%20Rights%20Reserved-lightgrey.svg?labelColor=475569)](LICENSE)

<table><tr>
<td style="background:#DCEEFB;border:1px solid #A9D6F5;border-radius:8px;padding:10px 14px;">🔵 <strong>ZERO-FABRICATION VERIFIED</strong> -- every figure in this repository is real and computed by this platform's own code; explicitly-labeled <code>ASSUMPTION</code>s are disclosed, never blended silently into a measured figure.</td>
<td style="background:#DFF5E6;border:1px solid #A6E5BC;border-radius:8px;padding:10px 14px;">🟢 <strong>PRODUCTION READINESS: RECOMMENDED</strong> -- real GitHub Actions status (verified 2026-09-16 at commit <code>6e223d8</code>): CI Pass · Code Quality Pass · CodeQL Pass · Docker Build &amp; Run Verification Pass · Secrets Management Verification Pass · 4x Independent Reproduction Pass.</td>
</tr></table>
...
<table>
<tr>
<td width="34%"><img src="00_Executive_Rollup_Report/platform_value_waterfall.png" alt="Real net value build-up waterfall across the 10 production-recommended problems, totaling $432.1M/cycle"></td>
...
<td width="33%"><img src="00_Executive_Rollup_Report/model_health_matrix.png" alt="Real model health matrix for all 14 problems -- 12 recommended, 2 foundational (no not-recommended systems in the current, re-verified result set)"></td>
</tr>
</table>

Every chart above is generated directly from this platform's own real results (not mockups). Two problems in this build were, at earlier iterations, honestly reported as not meeting their KPI target (Problem 7, then Problem 10) -- both were later re-verified after real fixes and are now RECOMMENDED FOR PRODUCTION, disclosed the same transparent way whichever way the result lands. The full Executive Rollup Report (linked above) has the complete set plus the narrative behind each number.

## System Architecture

<img src="docs/assets/system_architecture_diagram.svg" alt="AMEX RiskIQ platform architecture: the real Kaggle AMEX dataset flows through 14 phase-grouped problem notebooks (Phases 1-5), each deploying its own auth-protected FastAPI service, into a registry-driven BI aggregation layer (Notebooks 70-73) that rolls up into one executive dashboard reporting a real $432.1M/cycle platform net value, with GitHub Actions CI/CD wrapping the whole pipeline.">

The real Kaggle AMEX dataset feeds all 14 problem notebooks, grouped into 5 phases. Every problem deploys its own Dockerized, auth-protected FastAPI microservice (14 total, all Docker-Build-&-Run-Verified in CI). Each service's own real `summary_json` is picked up by Problem 14's registry-driven BI aggregation layer (Notebooks 70-73 -- `PRIOR_PROBLEMS_REGISTRY`, no problem-specific special-casing) and rolled up into the Executive Rollup Dashboard shown at the top of this README, gated by two hard-gating KPIs (`aggregation_completeness`, `aggregation_scope_correctness`) so the headline total can never silently double-count or mis-scope a problem. GitHub Actions CI/CD (CI, Code Quality, CodeQL, Docker Build & Run Verification, Secrets Management Verification) wraps every stage of this pipeline -- see the badges at the top of this page for real, current status.
...
| Credit-line limit optimization (Problem 10, current real result) | Net benefit $2,206,596.12/cycle, KPI target met (with CI), RECOMMENDED FOR PRODUCTION (an earlier iteration missed its KPI target and was honestly reported as not-recommended at the time; this is the current, re-verified result) |
...
| **Total platform net value across all 10 recommended value-creation problems** | **$432,132,848.85 / cycle** -- see the [Executive Rollup Report](00_Executive_Rollup_Report/AMEX_RiskIQ_Platform_Executive_Rollup_Report.docx) |
...
2. 📊 [Problem 2 — Risk Tier Classification Dashboard](https://rnanda19.github.io/AMEX_RiskIQ_Enterprise_Credit_Risk_Platform/02_Problem2_Risk_Tier_Classification/reports/financial_impact_reporting_packaging/risk_tier_classification_dashboard.html) — real tier population, bad-rate, chi-square/Cramér's V statistical validation dashboard (business_rule vs quantile methods)
...
**2026-09-16 hardening delta (Problem 10 re-verification + Problem 14 rollup refresh):**

- Real bug found and fixed: Notebook 73 (Problem 14's rollup) hardcoded "Problem 7 is a built, validated, but real not-recommended-for-production system" as fixed narrative text in 5 places (Word report, PDF, HTML dashboard chart captions, SMART suggestions, completion summary). That text had silently gone stale after Problem 7 was re-verified to RECOMMENDED, and would have gone stale again the moment Problem 10 was too — it also *named the wrong excluded problem* once Problem 10's own status changed. Replaced with logic that computes the real excluded/not-recommended set from the live data on every run, so this can never drift out of sync with the numbers again.
- Real bug found and fixed: Notebook 73's platform-value waterfall chart title hardcoded "Across 9 Production-Recommended Problems" instead of deriving the count from `INCLUDED_PROBLEMS`.
- Real bug found and fixed: the Problem 10 dashboard's sticky table header used the wrong scroll-ancestor offset (`top: 58px` against `.table-wrap`, not the page) plus a translucent background, causing header/row text overlap on scroll — fixed to `top: 0` with an opaque surface color.
- Real bug found and fixed: Notebook 57 (Problem 10) wrote its summary JSON without the `net_benefit_per_cycle_usd` / `meets_kpi_with_ci` / `recommended_for_production` fields Notebook 71's registry loader requires from the shared `artifacts/` location — would have silently excluded Problem 10 from every future rollup rerun.
