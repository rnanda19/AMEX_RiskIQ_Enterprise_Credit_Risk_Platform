#!/usr/bin/env bash
# Real, runnable demonstration of writing and reading a secret from a local
# dev-mode Vault (see docker-compose.vault.yml), using nothing but curl
# against Vault's real HTTP API -- so it needs no extra Python dependency
# in the 14 services this platform already ships. Exits non-zero on any
# real failure so it can gate a CI job (see
# .github/workflows/vault-verify.yml), not just print output and hope.
set -euo pipefail

VAULT_ADDR="${VAULT_ADDR:-http://127.0.0.1:8200}"
VAULT_TOKEN="${VAULT_TOKEN:-root-token-for-local-dev-only}"
SECRET_PATH="secret/data/amex/problem7"
SECRET_KEY="api_key"
